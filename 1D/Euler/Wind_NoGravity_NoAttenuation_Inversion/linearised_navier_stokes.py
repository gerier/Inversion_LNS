...
def dchi(reverse_U, obs_U, t, dt, index_receivers, max_obs):
    ...
    if t == 0 : 
        ...
        print(obs_U_t.p[index_receivers],reverse_U_t.p[index_receivers], max_obs[2])

    diff[2, index_receivers] = (
        obs_U_t.p[index_receivers] - reverse_U_t.p[index_receivers]) / max_obs[2]

    return diff


def get_adjoint_RHS(t, Ua, compute_velocity, M, dz, dt, reverse_U, observation_U, index_receivers, max_obs, order_DF=4):
    if order_DF == 4:
        DF_C = DF_C_4
        DF_DC = DF_DC_4_backward
    else:
    ...
    if compute_velocity:
        ...
        RHS[1, :-1] += M.gamma[1] * DF_C(pa_p0, dz, BC_pap0, False)        # grad ( gamma pa p0)
...
def get_kernels_expr(rho_a, v_a, p_a, rho_p, v_p, p_p, M, dtvp, dz, DF_C, DF_DC):
    ...
    K[0, :] += interpolation(M.v) * interp_vp * DF_C(v_a, dz, BC_va)    # v0 (vp . nabla) va
