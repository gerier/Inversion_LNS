...
implicit none
 ...
 ! Interpolate material properties and velocity onto the appropriate staggered-grid locations.
...
!-----------------------------------------------------------------------
! Pressure and density update + C-PML memory variables
!-----------------------------------------------------------------------

  ! Store previous pressure and density values
  p_old(:,:) = pressure(:,:)
  rhop_old(:,:) = rhop(:,:)

  ! Exchange ghost-cell values with neighboring MPI processes.
  ...
  do j = 1,NY_LOCAL
    do i = 1,NX_LOCAL
       ...
       ! Compute global indices and local neighbor indices.
       i_global = i + offset_i
       j_global = j + offset_j
       ...
       ! Compute derivatives vy with respect to y
       ...
       ! Compute derivatives of p0, p, rho0 and rhop with respect to x
       ...
       ! Compute derivatives of p0, p, rho0 and rhop with respect to y
       ...
       ! Apply the C-PML correction to the derivatives
       ...
       ! Update pressure and density fields
       pressure(i,j) = pressure(i,j) - (gamma_chemestry(i,j) * p0(i,j) * (value_dvx_dx + value_dvy_dy)) * DELTAT
  ...
  ! Inject the source
  ! ! Inject the pressure and density source.
  t = dble(it-1)*DELTAT
  ...
  if (type_source == 1) then ! plane wave case
    if (wavefront == 1 .and. i_rank == ISOURCE / NX_LOCAL) then
      i = ISOURCE - offset_i
      do j = 1,NY_LOCAL
        pressure(i,j) = pressure(i,j) + source_term * DELTAT * gamma_chemestry(i,j) * p0(i,j) / rho0(i,j)
  ...
  elseif (type_source == 2 ) then ! Point source
   ...
   else if (type_source ==3) then  ! Point source with Gaussian spatial spreading
      do j = 1,NY_LOCAL
       do i = 1,NX_LOCAL
          ...
          pressure(i,j) = pressure(i,j) + source_term * factor_ssf * DELTAT * gamma_chemestry(i,j) * p0(i,j) / rho0(i,j)
...
!--------------------------------------------------------
  ...
  ! Store the velocity fields in auxiliary arrays
  vx_old(:,:) = vx(:,:)
  vy_old(:,:) = vy(:,:)


  !  Send information to neighboring MPI processes
  ...
  do j = 1,NY_LOCAL
    do i = 1,NX_LOCAL
      ...
      ! Interpolate variables onto staggered-grid locations.
      ...
      ! Compute derivatives of pressure, density with respect to x
      ...
      ! Compute derivatives of windx, vx with respect to x
      if (windx(i,j) >= 0) then
     ...
     ! Compute derivatives of rho0 with respect to y (evaluating in vx point)
     ...
     ! Intermediate computations: (v0 . nabla) v'_x ; (v' . nabla) v0_x
     value_vdwindx = vy_half_x_half_y * value_dwindx_dy
     value_v0dvx   = windx(i,j)       * value_dvx_dx


     ! Update the horizontal velocity wavefield
  ...
  do j = 1,NY_LOCAL
    do i = 1,NX_LOCAL
      ...
      ! Interpolate density onto the appropriate staggered-grid location
      ...
      ! Compute derivatives of pressure and density with respect to y
      ...
      ! Compute advection terms: (v0 . nabla) v_y ; (v' . nabla) v0_y
      value_v0dvy  = windx_half_x_half_y * value_dvy_dx


      ! Update the vertical velocity wavefield
  ...
  if (USE_PML_YMIN .and. j_rank == 0) then ! PML
      vx(:,-1:1) = ZERO
      vy(:,-1:1) = ZERO
  elseif (j_rank == 0) then ! Reflective boundary condition at the ground.
     do j=-1,1
        vx(:,j) = -vx(:,3-j)
        vy(:,j) = -vy(:,2-j)
     enddo
  ...
  ! Record seismograms
  do irec = 1,NREC
     ! Note that the two components of the velocity vector are not defined at the same location
     ! on the staggered grid. Therefore, they are recorded at slightly different positions:
     ! vy is staggered by half a grid cell in both the x- and y-directions with respect to vx.
  ...
  ! Save the pressure perturbation at the source when delay-time observations are used.
  if (observation_from_file == 2 .and. i_rank == ISOURCE / NX_LOCAL .and. j_rank == JSOURCE / NY_LOCAL) then
   ...
   ! Update global maxima used for normalized snapshots.
   ...
   if ((mod(it,IT_DISPLAY) == 0 .or. it == 5) .and. (save_sismos)) then
     ...
     if ((.not.validation) .and. rank==0) then
         ...
         ! Print the maximum pressure and the maximum of the norm of the velocity
         ...
         ! Stop the simulation if numerical instability is detected.
