subroutine forwardproblem(p0,rho0,windx,windy,kappa_unrelaxed, it_start, it_end, type_number)
 !
...
! output information
  if ((mod(it,IT_DISPLAY) == 0 .or. it == 5) .and. (save_sismos)) then
    ...
    call MPI_REDUCE(maxval(abs(pressure(1:NX_LOCAL,1:NY_LOCAL))), &
        pressurenorm,1,MPI_DOUBLE_PRECISION,MPI_MAX,0,MPI_COMM_WORLD,code)
