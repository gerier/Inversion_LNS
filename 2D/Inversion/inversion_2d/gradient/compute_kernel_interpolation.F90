...
! this fails if we cross the end of the month
  ...
  do it=1,NSTEP
    ...
    if (NSTEP-it == NSTEP-1 .or. modulo(NSTEP-it,NSTEP/NFRAMES) == NSTEP/NFRAMES-1 ) then
        ...
        call forwardproblem(p0_prior,rho0_prior,windx_prior,windy_prior,kappa_unrelaxed_prior,time_last_frame, NSTEP-it,1)
    endif
 ...
 if (USE_PML_XMIN) then ! if USE_PML_XMIN, we have also USE_PML_XMAX
    if (i_rank == 0) then
       do jj=1,NY_LOCAL
  ...
  if (USE_PML_YMIN) then ! if USE_PML_YMIN, we have also USE_PML_YMAX
   if (j_rank == 0) then
    do ii=1,NX_LOCAL
