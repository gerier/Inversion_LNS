...
def DF_DC_2(U, dz, BC, sense):
    diff_U = np.zeros(len(U))
    
    for i in range(len(U)):
        # depending on the sense, the scheme is forward or backward
        if sense[i] > 0 :
            if i < 1:
...
def DF_DC_2_backward(U0, dz, BC, sense=1):
    U = np.append([BC[1]], U0)
    U = U[1:] - U[:-1]
    return U / dz

def DF_DC_2_forward(U0, dz, BC, sense=0):
    U = np.append(U0, [BC[-2]])
    U = U[1:] - U[:-1]
    return U / dz
...
def DF_DC_4(U, dz, BC, sense):
    ...
    for i in range(len(U)):
        ...
        if sense[i] > 0 :
            ...
            if i == len(U) - 1:
                diff_U[i] = -BC[-1]/6 + BC[-2] - U[i]/2 - U[i-1]/3  
            elif i == len(U) - 2 :
                diff_U[i] = -BC[-2]/6 + U[i+1] - U[i]/2 - U[i-1]/3
            elif i == 0 :
...
def DF_DC_4_backward(U0, dz, BC, sens=1):
...
def DF_DC_4_forward(U0, dz, BC, sense=0):
    aux_U = np.empty(len(U0)+3) 
    aux_U[1:-2] = U0
    aux_U[0] = BC[1]
    aux_U[-2:] = BC[-2:]
...
def EE(f_dt, U_t, T_init, Tmax, z, propagation_type, *args):
    # we suppose here at initialisation that rho and p in given at time t=0, and 
    # rho and v are given at time t=1/2
    ...
    for t in t_tab:
        ...
        if propagation_type == "forward" :
            # first iter : update rho and p
            aux = deepcopy(U_t)
            tndemi = f_dt(t, aux, *args)
            U_t.p +=   tndemi.p
            #print(U_t.p[-1])
            # second iter : update v
            aux = deepcopy(U_t)
            tn= f_dt(t+dt/2, aux, *args)
            U_t.v +=  tn.v
