...
! total number of grid points in each direction of the grid
  ...
  integer, parameter :: NX_LOCAL = NX / NPROC 
...
! for seismograms
  ...
  double precision, dimension(0:NX+1, 0:NY+1) :: K_rho0, K_windx, K_windy, K_p0
  ...
  ! PML variables for checkpointing
      double precision, dimension(-1:NX_LOCAL+2,0:NY+1,NFRAMES) ::                &
  ! for equation on rhop and pressure
 ...
 ! number of values to send or receive
  integer, parameter :: number_of_values = 2*(NY+2)

  integer :: nb_procs,rank,code,rank_cut_plane,i_global,offset_i,i2begin,iminus1end !kmin,kmax
