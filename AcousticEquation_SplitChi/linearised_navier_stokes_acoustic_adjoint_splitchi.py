...
def dchi(reverse_U, obs_U, t, dt, index_receivers, max_obs, which_chi):
...
def get_adjoint_RHS(Ustar, t, previous_Ustar, U0, T0, g, l, mu, kappa, gamma, Cv, dz, dt, source, reverse_U, observation_U, index_receivers, max_obs, which_chi, order_DF=4):
    if order_DF == 4:
        DF_C = DF_C_4
    else :
        DF_C = DF_C_2
...
def get_kernels(hist_adjoint, hist_backprop, U0, T0, kappa, gamma, Cv, dt, dz, order_DF=4):
    ...
    for t in range(len(hist_adjoint)-1):
        ...
        # create variable to be more lisible
        #rho_a = hist_adjoint[t].rho     #  at time n+1/2
        #rho_p = hist_backprop[t].rho    #  at time n+1/2

        #p_a = hist_adjoint[t].p         #  at time n+1/2
        #p_p = hist_backprop[t].p        #  at time n+1/2

        #v_a = (hist_adjoint[t].v + hist_adjoint[t+1].v)              # mean at time n
        #v_p = (hist_backprop[t].v + hist_backprop[t+1].v)            # mean at time n
