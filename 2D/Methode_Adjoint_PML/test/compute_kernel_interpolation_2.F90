...
! INITIALISATION
  ...
  ! Make a first simulation with the apriori model to save frames 
  call save_frames()
  ...
  ! Start the kernel computation (that consists in correlation between adjoint and backward field)
  ! Compute for each time the correlation and add to the kernel variables
  do it=1,NSTEP
  
    ! initialise old variables 
    ! these variables are useful since the leap frog scheme and the derivative 
    ! in time in the density kernel expression
    rhop_old(:,:) = rhop(:,:)
    rhoa_old(:,:) = rhoa(:,:)
    p_old(:,:)  = pressure(:,:)
    pa_old(:,:) = pa(:,:)
    ...
    ! to compute an exact backward simulation, we use checkpointing
    ! First select the best frame as starting point
    ! Then, launch the simulation until the desired time step
    call load_frame(NSTEP-it, it_start_frame, pressure, rhop, vx, vy)
    call forwardproblem(pressure, rhop, vx, vy, p0, rho, v0x, v0y, kappa_unrelaxed, it_start_frame, NSTEP-it,3)
    
    ! compute the adjoint 
    call compute_adjoint(pa, rhoa, vax, vay, it)
    
    ! draw an image f the adjoint field
    ...
    ! interpolate field, to have each variable at exactly the same moment 
    ...
    ! compute the contribution of the kernel at time it
