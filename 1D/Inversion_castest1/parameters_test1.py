...
index_source1 = int(15*1000/h+nb_index_neg)
...
index_receivers = (np.arange(12,41,3) *1000/h).astype('int') 
...
factor_rho = 0
factor_c = -0.01
obs_start = int(16*1000/h + nb_index_neg) #int(20*1000/h + nb_index_neg)
...
GT_rho0[obs_start:obs_end] *= 1.+ factor_rho * np.exp(  - (z[obs_start:obs_end] - ( z[obs_end]+z[obs_start])/2)**2 /  500/(z[obs_end]-z[obs_start])   )

c2 = c**2
GT_c2 = deepcopy(c2)
GT_c2[obs_start:obs_end] *= 1.+ factor_c * np.exp(  - (z[obs_start:obs_end] - ( z[obs_end]+z[obs_start])/2)**2 /  500/(z[obs_end]-z[obs_start])   )


factor_rho = 0.0
factor_c = 0.008
obs_start = int(29*1000/h + nb_index_neg) #int(20*1000/h + nb_index_neg)
obs_end = int(38*1000/h + nb_index_neg) #int(30/h*1000 + nb_index_neg)

#GT_rho0[obs_start:obs_end] *= 1 + factor_rho * np.exp(  - (z[obs_start:obs_end] - ( z[obs_end]+z[obs_start])/2)**2 /  50/(z[obs_end]-z[obs_start])   )
GT_c2[obs_start:obs_end] *= 1 + factor_c * np.exp(  - (z[obs_start:obs_end] - ( z[obs_end]+z[obs_start])/2)**2 /  300/(z[obs_end]-z[obs_start])   )
#GT_c2 *= 1.05
...
ax[0].plot(z/1000, M.rho)
ax[0].set_ylabel("Density")
ax[1].plot(z/1000, GT_M.p)
ax[1].plot(z/1000, M.p)
ax[1].set_ylabel("Pressure")
ax[2].plot(z/1000, np.sqrt(GT_c2))
ax[2].plot(z/1000, np.sqrt(c2))
