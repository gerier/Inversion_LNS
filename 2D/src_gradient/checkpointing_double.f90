...
subroutine load_frame(it_time, it_time_frame)
  ...
  ! define the number of time step between 2 frames
  step_frames = NSTEP / NFRAMES 
  
  ! define the number of time step between 2 local frames
  step_local_frames = step_frames / N_LOC_FRAMES 
  
  if (modulo(it_time, step_frames) < step_local_frames) then ! is a global frame?
...
subroutine save_frames()
 ...
 do it_frame=1,NFRAMES
   ...
   ! save the new frame (and the memory variables for PML) 
...
subroutine save_local_frames(it_time)
 ...
 do it_frame=1,N_LOC_FRAMES
   ...
   ! save the new frame (and memory variables for PML)
