...
def optimisation(x0, f0, gradf0, hessf0, c1, c2, argf=[],argg=[], argh=[], alpha_max=20, maxiter=1000, tol_x=1e-8, tol_df=1e-8, true_hessian=False, regularisation=0, plot_iter=None, save_info=None,type_gradient=2,type_regul=0):
    ...
    if regularisation :
        ...
        if type_regul == 0:
            f = lambda x,argf : f0(x, *argf[3:]) + gamma * sum((x-argf[2])**2) 
            f2 = lambda x,argf : [f0(x, *argf[3:]), gamma * sum((x-argf[2])**2)]

            def gradf(x,argg):
                grad = gradf0(x, *argg[3:])
                grad[0][:argg[1]] = grad[0][:argg[1]] + 2 * gamma * (x[:argg[1]] - argg[2][:argg[1]])
                grad[0][argg[1]:2*argg[1]] = grad[0][argg[1]:2*argg[1]] + 2 * gamma * (x[argg[1]:2*argg[1]] - argg[2][argg[1]:2*argg[1]])
                grad[0][2*argg[1]:] = grad[0][2*argg[1]:] + 2 * gamma * (x[2*argg[1]:] - argg[2][2*argg[1]:])
                return grad
