...
def F(z, t, f0, index_source,dt=None,signal=None,fcutoff=None):
    # initialisation
    f = np.zeros((3, len(z)))
    # define source term
    source = get_source_t_x(z, t, f0, index_source,dt,signal,fcutoff)
    # compute the contribution on quantity of mvt
    f[2, :] = source
    return f

def get_source(t, f0,dt=None,signal=None,fcutoff=None):
    t0 = 1.2/f0
...
def get_source_t_x(z, t, f0, index_source,dt=None,signal=None,fcutoff=None):
    # define source time function
    time_source = get_source(t, f0,dt,signal,fcutoff)
...
def get_RHS(t, U1, compute_velocity, M, dz, dt, source, order_DF=4):
    ...
    if compute_velocity :
        ...
        RHS_c[0, :] += F_source * (M.rho / M.gamma / M.p) 
...
def dchi(reverse_U, obs_U, t, dt, index_receivers, norm_info, n):
    diff = np.zeros((3, n))
...
def get_adjoint_RHS(t, Ua, compute_velocity, M, dz, dt, index_receivers, reverse_U, observation_U, norm_info, order_DF=4):
    if order_DF == 4:
        DF_C = DF_C_4
        DF_DC = DF_DC_4_backward
    else:
    ...
    # source
    Fadjoint = dchi(reverse_U, observation_U, t, dt, index_receivers, norm_info, len(Ua.rho))
    # boundary conditions
    BC_pa = [0,0,0,0]
    BC_va = [0,0,0,0]
    BC_v0 = [M.v[0], M.v[0], M.v[-1], M.v[-1]]
