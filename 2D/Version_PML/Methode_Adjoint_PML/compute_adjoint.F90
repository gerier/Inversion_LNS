subroutine compute_adjoint(it_step)
  ...
  double precision, dimension(0:NX+1,0:NY+1) :: rhoa_old, pa_old, vax_old, vay_old
  ...
  do j=1,NY
   do i=1,NX
    ...
    eq1_memory_drho0_dx_adj(i,j)     = b_x(i) * eq1_memory_drho0_dx_adj(i,j)     + a_x(i) * value_drho0_dx
    eq1_memory_dp0_dx_adj(i,j)       = b_x(i) * eq1_memory_dp0_dx_adj(i,j)       + a_x(i) * value_dp0_dx
    eq1_memory_drhoarho0_dx_adj(i,j) = b_x(i) * eq1_memory_drhoarho0_dx_adj(i,j) + a_x(i) * value_drhoarho0_dx
    eq1_memory_dp0pa_dx_adj(i,j)     = b_x(i) * eq1_memory_dp0pa_dx_adj(i,j)     + a_x(i) * value_dp0pa_dx 
    ...
    eq1_memory_dvax_dy_adj(i,j) = b_y(j) * eq1_memory_dvax_dy_adj(i,j) + a_y(j) * value_dvax_dy
    ...
    eq1_memory_dwindx_dx_adj(i,j) = c_x(i) * value_dwindx_dx
    eq1_memory_dvax_dx_adj(i,j) = b_x(i) * eq1_memory_dvax_dx_adj(i,j) + a_x(i) * value_dvax_dx
    ...
    eq1_memory_dwindy_dx_adj(i,j) = c_x(i) * value_dwindy_dx
    ...
    eq1_memory_drho0_dy_adj(i,j)     = b_y_half(j) * eq1_memory_drho0_dy_adj(i,j)     + a_y_half(j) * value_drho0_dy
    eq1_memory_dp0_dy_adj(i,j)       = b_y_half(j) * eq1_memory_dp0_dy_adj(i,j)       + a_y_half(j) * value_dp0_dy
    eq1_memory_drhoarho0_dy_adj(i,j) = b_y_half(j) * eq1_memory_drhoarho0_dy_adj(i,j) + a_y_half(j) * value_drhoarho0_dy
    eq1_memory_dp0pa_dy_adj(i,j)     = b_y_half(j) * eq1_memory_dp0pa_dy_adj(i,j)     + a_y_half(j) * value_dp0pa_dy
    ...
    eq1_memory_dvay_dx_adj(i,j) = b_x_half(i) * eq1_memory_dvay_dx_adj(i,j) + a_x_half(i) * value_dvay_dx
    ...
    eq1_memory_dwindy_dy_adj(i,j) = c_y_half(j) * value_dwindy_dy
    eq1_memory_dvay_dy_adj(i,j) = b_y_half(j) * eq1_memory_dvay_dy_adj(i,j) + a_y_half(j) * value_dvax_dx
    ...
    eq1_memory_dwindx_dy_adj(i,j) = c_y_half(j) * value_dwindx_dy


    !
    value_drho0_dx     = value_drho0_dx     * one_over_K_x(i) + eq1_memory_drho0_dx_adj(i,j)  
    value_dp0_dx       =  value_dp0_dx      * one_over_K_x(i) + eq1_memory_dp0_dx_adj(i,j)
    value_drhoarho0_dx = value_drhoarho0_dx * one_over_K_x(i) + eq1_memory_drhoarho0_dx_adj(i,j)
    value_dp0pa_dx     = value_dp0pa_dx     * one_over_K_x(i) + eq1_memory_dp0pa_dx_adj(i,j) 
    
    value_dvax_dy      = value_dvax_dy      * one_over_K_x(i) + eq1_memory_dvax_dy_adj(i,j)

    value_dwindx_dx    = value_dwindx_dx    * one_over_K_x(i) + eq1_memory_dwindx_dx_adj(i,j)
    value_dvax_dx      = value_dvax_dx      * one_over_K_x(i) + eq1_memory_dvax_dx_adj(i,j) 

    value_dwindy_dx    = value_dwindy_dx    * one_over_K_x(i) + eq1_memory_dwindy_dx_adj(i,j)
    
    value_drho0_dy     = value_drho0_dy     * one_over_K_y_half(j) + eq1_memory_drho0_dy_adj(i,j)
    value_dp0_dy       = value_dp0_dy       * one_over_K_y_half(j) + eq1_memory_dp0_dy_adj(i,j)  
    value_drhoarho0_dy = value_drhoarho0_dy * one_over_K_y_half(j) + eq1_memory_drhoarho0_dy_adj(i,j) 
    value_dp0pa_dy     = value_dp0pa_dy     * one_over_K_y_half(j) + eq1_memory_dp0pa_dy_adj(i,j)  

    value_dvay_dx      = value_dvay_dx      * one_over_K_y_half(j) + eq1_memory_dvay_dx_adj(i,j) 

    value_dwindy_dy    = value_dwindy_dy    * one_over_K_y_half(j) + eq1_memory_dwindy_dy_adj(i,j) 
    value_dvax_dx      = value_dvax_dx      * one_over_K_y_half(j) + eq1_memory_dvay_dy_adj(i,j)
       
    value_dwindx_dy    = value_dwindx_dy    * one_over_K_y_half(j) + eq1_memory_dwindx_dy_adj(i,j)
  ...
  do j=1,NY
   do i=1,NX
    ...
    eq2_memory_dpawindx_dx_adj(i,j)   = b_x_half(i) * eq2_memory_dpawindx_dx_adj(i,j) + a_x_half(i) * value_dpawindx_dx
    eq2_memory_drhoawindx_dx_adj(i,j) = b_x_half(i) * eq2_memory_drhoawindx_dx_adj(i,j) + a_x_half(i) * value_drhoawindx_dx
    ...
    eq2_memory_dpawindy_dy_adj(i,j)   = b_y(j) * eq2_memory_dpawindy_dy_adj(i,j) + a_y(j) * value_dpawindy_dy
    eq2_memory_drhoawindy_dy_adj(i,j) = b_y(j) * eq2_memory_drhoawindy_dy_adj(i,j) + a_y(j) * value_drhoawindy_dy
    ...
    eq2_memory_dwindx_dx_adj(i,j) = b_x_half(i) * eq2_memory_dwindx_dx_adj(i,j) + a_x_half(i) * value_dwindx_dx 
    eq2_memory_dvax_dx_adj(i,j)   = b_x_half(i) * eq2_memory_dvax_dx_adj(i,j)   + a_x_half(i) * value_dvax_dx
    ...
    eq2_memory_dwindy_dy_adj(i,j) = b_y(j) * eq2_memory_dwindy_dy_adj(i,j) + a_y(j) * value_dwindy_dy
    eq2_memory_dvay_dy_adj(i,j)   = b_y(j) * eq2_memory_dvay_dy_adj(i,j)   + a_y(j) * value_dvay_dy
    ...
    eq2_memory_dwindy_dx_adj(i,j) = b_x_half(i) * eq2_memory_dwindy_dx_adj(i,j) + a_x_half(i) * value_dwindy_dx 
    ...
    eq2_memory_dwindx_dy_adj(i,j) = b_y(j) * eq2_memory_dwindx_dy_adj(i,j) + a_y(j) * value_dwindx_dy
    
    
    !   
    value_dpawindx_dx   = value_dpawindx_dx   * one_over_K_x_half(i) + eq2_memory_dpawindx_dx_adj(i,j) 
    value_drhoawindx_dx = value_drhoawindx_dx * one_over_K_x_half(i) + eq2_memory_drhoawindx_dx_adj(i,j)
    value_dpawindy_dy   = value_dpawindy_dy   * one_over_K_y(j)      + eq2_memory_dpawindy_dy_adj(i,j)
    value_drhoawindy_dy = value_drhoawindy_dy * one_over_K_y(j)      + eq2_memory_drhoawindy_dy_adj(i,j)

    value_dwindx_dx     = value_dwindx_dx * one_over_K_x_half(i) + eq2_memory_dwindx_dx_adj(i,j)
    value_dvax_dx       = value_dvax_dx   * one_over_K_x_half(i) + eq2_memory_dvax_dx_adj(i,j)
    value_dwindy_dy     = value_dwindy_dy * one_over_K_y(j)      + eq2_memory_dwindy_dy_adj(i,j)
    value_dvay_dy       = value_dvay_dy   * one_over_K_y(j)      + eq2_memory_dvay_dy_adj(i,j)
    
    value_dwindy_dx     = value_dwindy_dx * one_over_K_x_half(i) + eq2_memory_dwindy_dx_adj(i,j)
    value_dwindx_dy     = value_dwindx_dy * one_over_K_y(j)      + eq2_memory_dwindx_dy_adj(i,j)
...
subroutine compute_adjoint_source(adjoint_source_term, it_step)
 ...
 use parameters, only : sispressure_true, sispressure_prior, norm_pressure_true, ix_rec, iy_rec, NREC, NX, NY, NSTEP, TINYVAL
 double precision, dimension(0:NX+1, 0:NY+1) :: adjoint_source_term
  ...
  adjoint_source_term(:,:) = 0.0d0
   ...
   do irec=1,NREC
       adjoint_source_term(ix_rec(irec), iy_rec(irec)) = &
                (sispressure_true(NSTEP-it_step+1,irec) - sispressure_prior(NSTEP-it_step+1,irec))
      if (abs(adjoint_source_term(ix_rec(irec), iy_rec(irec)))  < TINYVAL) then
        adjoint_source_term(ix_rec(irec), iy_rec(irec)) = 0.0d0
        endif

      adjoint_source_term(ix_rec(irec), iy_rec(irec)) = adjoint_source_term(ix_rec(irec), iy_rec(irec)) / norm_pressure_true
