...
def compute_gradient(M, it_start, it_end, dt, h, x_size, sponge_layer, source, index_receivers, history_obs, norm_info):
    ...
    K = np.zeros((3, len(adjoint_prec.rho)))
    for it in range(1,nstep):
        ...
        _, adjoint = time_scheme(get_adjoint_RHS, U_start_adjoint, it, it+1, dt, index_receivers, "adjoint", sponge_layer, M, h, history_reverse, history_obs[-it], norm_info,it_end,source)
        U_start_adjoint = deepcopy(adjoint)

        # compute kernel
        source_with_time = [(it_end - it+1-0.5)*dt] + source
