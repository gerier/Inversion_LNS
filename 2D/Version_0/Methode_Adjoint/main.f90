...
! i.e. when switching to a fourth-order spatial scheme one needs a time step that is about 0.707 / 0.606 = 1.167 times smaller.
  ...
  vx(:,:)          = ZERO
  vy(:,:)          = ZERO
  call forwardproblem(pressure, rhop, vx, vy, p0_obs, rho_obs, v0x_obs, v0y_obs, kappa_unrelaxed_obs,  1, NSTEP,33) 
  ...
  !do it=1,NSTEP
  !  use_checkpoint = .True.
  !  call load_last_frame(NSTEP-it, it_last_frame, pressure, rhop, vx, vy)
  !  call forwardproblem(pressure, rhop, vx, vy, p0, rho, v0x, v0y, kappa_unrelaxed, it_last_frame, NSTEP-it)
  !enddo
  ...
  !call compute_adjoint(pa, rhoa, vax, vay, it_step)
  !call compute_kernel()
