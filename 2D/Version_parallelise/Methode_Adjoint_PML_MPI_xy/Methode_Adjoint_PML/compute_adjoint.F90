  ...
  adjoint_source_term(:,:) = 0.0d0
   ...
   do irec=1,NREC
     ...
     if (i_rank == ix_rec(irec)/NX_LOCAL .and. j_rank == iy_rec(irec)/NY_LOCAL) then
       ...
       adjoint_source_term(i,j) = (sispressure_true(NSTEP-it_step+1,irec) - sispressure_prior(NSTEP-it_step+1,irec))
      if (abs(adjoint_source_term(i,j))  < TINYVAL) then
        adjoint_source_term(i,j) = 0.0d0
      endif
      
      !print *,sispressure_true(NSTEP-it_step+1,irec),sispressure_prior(NSTEP-it_step+1,irec),adjoint_source_term(i,j), i,j,it_step
      adjoint_source_term(i,j) = adjoint_source_term(i,j) / norm_pressure_true
