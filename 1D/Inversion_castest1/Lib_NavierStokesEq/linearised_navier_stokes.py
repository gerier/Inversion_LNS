...
class LNS_Model:
    ...
    def plot(self, abs, title):
        ...
        n = int(len(abs))
        abs_km = abs / 1000
        ax[0].plot(abs_km[:n], self.rho[:n])
        ax[1].plot(abs_km[:n], self.p[:n])
        ax[2].plot((abs_km[1:n]+abs_km[:n-1])/2, self.v[:n-1])
        ax[3].plot(abs_km[:n], np.sqrt(self.p[:n] * self.gamma / self.rho[:n]))
...
def F(z, t, f0, Tmax,index_source,dt=None,signal=None,fcutoff=None):
    # define source time function
    time_source = get_source(t, f0,Tmax, dt,signal,fcutoff)
...
def get_source(t, f0,Tmax,dt=None,signal=None,fcutoff=None):
    t0 = 1.2/f0
    ...
    if False : 
        ...
        fs = 1/dt
        filtered = butter_lowpass_filter(signal, fcutoff, fs, order=1)
        return filtered[int(t/dt)]
    else:
        stf =  - 2* pi**2 * f0**2 * (t-t0) * np.exp(- pi**2 * f0**2 * (t-t0)**2) 
        return stf

#%% Utils
    
def apodisation(t,t0,Tmax):
    damping = 1
    if (t < t0 / 2):
        damping = 0.5 - 0.5 * np.cos(2*np.pi*t/(t0))
    elif (t > Tmax - t0/2):
        damping = 0.5 - 0.5 * np.cos(2*np.pi*(Tmax-t)/(t0))
    return damping
...
def get_RHS(t, U1, compute_velocity, M, dz, dt, source, order_DF=4):
    ...
    arg_fn_source = [source[0]] + [t] + source[1:]
    F_source = F(*arg_fn_source) 
    ...
    BC_v1 = [0, 0, 0, 0]
    BC_p1 = [0, 0, 0, 0]
    BC_rho0 = [M.rho[0],  M.rho[0],  M.rho[-1],  M.rho[-1]]
    BC_v0 = [M.v[0],    M.v[0],    M.v[-1],    M.v[-1]]
    BC_p0 = [M.p[0],    M.p[0],    M.p[-1],    M.p[-1]]
    ...
    if compute_velocity :
        ...
        interp_g = (M.g[1:]+M.g[:-1])/2
        arg_fn_source = [source[0]] + [t+0.5*dt] + source[1:]
        F_source = F(*arg_fn_source)
        interp_source = F_source[1:] #(F_source[1:]+F_source[:-1])/2

        # compute the contribution on qunatity of mvt
        RHS_c[1, :-1] = - interp_rho0 * M.v * DF_DC(U1.v, dz, BC_v1, M.v)                  # div( rho0v0 x vp)
        RHS_c[1, :-1] -= DF_C(U1.p, dz, BC_p1, False)                                        # grad p1
        RHS_c[1, :-1] -= (interp_rho0 * U1.v + interp_rho1 * M.v ) * DF_DC(M.v, dz, BC_v0, M.v) # (rhop v0 + rho0 vp) . nabla) v0
        RHS_c[1, :-1] -= interp_rho1 * interp_g 
        ...
        RHS_c[2, :] += F_source * M.gamma * M.p / M.rho 
...
def dchi(reverse_U, obs_U, t, dt, index_receivers, norm_info, n, source):
    diff = np.zeros((3, n))

    if norm_info['choice'] == 1 :
        diff[2, index_receivers] = (obs_U - reverse_U) / norm_info["norm_obs"]**2

    elif norm_info['choice'] == 2 :
        if norm_info["calc"]:
            if norm_info['ord'] == 2:
                for r in range(len(index_receivers)):
                    diff[2,index_receivers[r]] = - (obs_U[r] / norm_info["norm_obs"][r]**2 + reverse_U[r] / norm_info["norm_calc"][r]**2) / norm_info["norm_calc"][r]**2
            else:
                print("ERROR: not implemented - how to compute the derivative in that case")
                sys.exit()
        else: 
            for r in range(len(index_receivers)):
                diff[2, index_receivers[r]] = (obs_U[r] - reverse_U[r])/norm_info["norm_obs"][r]**2
        ...
        if norm_info["calc"]:
            ...
            for r in range(len(index_receivers)):
                diff[2, index_receivers[r]] = (obs_U[r] - reverse_U[r])/norm_info["norm_obs"][round(t//dt)][r]**2


    damping = apodisation(t,1.2/source[1],source[2])
    diff = diff * damping  
    return diff


def get_adjoint_RHS(t, Ua, compute_velocity, M, dz, dt, source, index_receivers, reverse_U, observation_U, norm_info, it_end, order_DF=4):
    if order_DF == 4:
        DF_C = DF_C_4
        DF_DC = DF_DC_4_forward
    else:
        DF_C = DF_C_2
        DF_DC = DF_DC_2_forward
    ...
    # source
    Fadjoint = dchi(reverse_U, observation_U, t, dt, index_receivers, norm_info, len(Ua.rho),source)
    # boundary conditions
    BC_pa = [0,0,0,0]
    BC_va = [0,0,0,0]
    BC_v0 = [M.v[0], M.v[0], M.v[-1], M.v[-1]]
    ...
    # source
    arg_fn_source = [source[0]] + [(it_end*dt) + 0.5*dt - t] + source[1:]
    F_source = F(*arg_fn_source)
    interp_source = (F_source[1:]+F_source[:-1])/2

    if compute_velocity:
        # contribution on va
        RHS[1, :-1] += interp_rho0 * DF_C(Ua.rho, dz, BC_rhoa, False)    # rhoa grad rho0
        RHS[1, :-1] -= interp_pa * DF_C(M.p, dz, BC_p0, False)           # pa grad p0
        RHS[1, :-1] += DF_C(M.gamma * pa_p0, dz, BC_gammapap0, False)    # grad ( gamma pa p0)
        #RHS[1, :-1] -= interp_rho0 * DF_DC(M.v, dz, BC_v0, M.v) * Ua.v   # rho0 (grad v0) va
        RHS[1, :-1] +=  DF_DC(interp_rho0 * M.v * Ua.v, dz, BC_va, M.v)   # rho0 (v0 . nabla) va    

    else : 
        interp_va = interpolation(Ua.v)
...
def get_kernels_expr(rho_a, v_a, p_a, rho_p, v_p, p_p, M, dtvp, dz, source, DF_C, DF_DC):
    ...
    arg_source = [source[1],source[0]] + source[2:]
    f_source = F(*arg_source)
    ...
    # kernel in rho0
    K[0, :] -= DF_DC(interp_vp * rho_a, dz, BC_rhoavp, M.v)    # div(vp rhoa)
    K[0, :] += rho_a * DF_C(v_p, dz, BC_vp)                             # vp grad rhoa
    K[0, :] -= interpolation(dtvp * v_a)                                # va dt vp
    K[0, :] += interp_v0 * interp_va * DF_C(v_p, dz, BC_vp)    # v0 (vp . nabla) va
    K[0, :] += interp_vp  * interp_va * DF_C(M.v, dz, BC_v0)            # va (v' . nabla) v0 
    K[0, :] += f_source * p_a * (M.gamma * M.p) / M.rho**2
    #K[0, :] -= interp_v0 * DF_C(v_a * v_p, dz, BC_vavp)

    # kernel in v0
    K[1, :-1] -= interp_rhop * DF_C(rho_a, dz, BC_rhoa, False)
    #K[1, :-1] -= interp_rho0 * v_p * DF_DC(v_a, dz, BC_va, M.v)    # rho0 ma grad vp
    K[1, :-1] += interp_rhop * v_a * DF_DC(M.v, dz, BC_v0, M.v)    # rho0 ma grad vp
    K[1, :-1] += v_a * interp_rho0 * DF_DC(v_p, dz, BC_vp, M.v) 
    ...
    # kernel in p0
    K[2, :] = p_a * (M.gamma-1) * DF_C(v_p, dz, BC_vp)  # pa gamma div vp
    K[2, :] -= interp_vp * DF_DC(p_a, dz, BC_pavp)   # div(pa vp)
    K[2, :] -= f_source * p_a * (M.gamma ) / M.rho 
