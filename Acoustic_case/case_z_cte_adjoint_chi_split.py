...
U_tmax = deepcopy(U_end)
t_start, U_start, history_adjoint = time_scheme(get_adjoint_RHS, U_end*0, T_init, Tmax, z, "adjoint", U0, T0, g, 0, 0, 0, gamma, Cv, h, dt, source, history_reverse, history_obs, index_receivers, which_chi, max_obs)
...
def chi(U0, history_obs, index_receivers, which_chi, max_obs, dt, name_file):
    # init
    CHI = np.zeros(3)
    # get Ucalc
    U1 = deepcopy(U0) * 0 
...
start_k = 10+nb_index_neg
end_k = 210+nb_index_neg
drho = 0.5 * U0.rho[0]
chi_rho = chi(U0, history_obs, index_receivers, which_chi, max_obs, dt, './Save_Kernel_info/model_m0')
...
for it,i in enumerate(range(start_k, end_k+1,5)):
    ...
    chi_rho_i = chi(U0_rho_di, history_obs, index_receivers, which_chi, max_obs, dt, './Save_Kernel_info/model_m_i_'+str(i))
    # compute dX
